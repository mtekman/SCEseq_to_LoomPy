  ...
  def r2py_sceseq(obj: SexpS4) -> AnnData:
      with localconverter(default_converter):
          ...
          def getsub(names_array, parse=True):
              if type(names_array) is str:
                  names_array = names_array.split('_')
              ## First is the slot name, rest are indices
              res = subget(slots[names_array[0]], names_array[1:])
              if not parse:
                  return(res)

              if res is rpy2.rinterface.NULL:
                  res = None
              elif len(res) == 1:
                  res = res[0]
              elif len(res) > 1:
                  if 'Matrix' in res.__class__.__name__:
                      res = extract_matrix(res)
                  elif 'Vector' in res.__class__.__name__:
                      if hasattr(res, "names"):
                          if res.names is rpy2.rinterface.NULL:
                              res = extract_array(res)
                          else:
                              res = extract_namedvector(res)
                      else:
                          res = extract_array(res)
                  else:
                      print("Panic", names_array)
          ...
          # obs
          counts = getsub(["counts"])  # libsize
          cluster_kpart = getsub("cluster_kpart")
          cluster_clb_partition = getsub("cluster_clb_partition")
          cluster_clb_result_partition = getsub("cluster_clb_result_partition")
          cluster_clb_result_result_pamobject_clustering = getsub("cluster_clb_result_result_pamobject_clustering")
          ...
          cluster_singlevals = [
              "gap_n", "gap_B",
              "clb_nc", "clb_nccl", "clb_clustermethod", "clb_B", "clb_noisemethod",
              "clb_bootmethod", "clb_multipleboot",
              "clb_dissolution", "clb_recover",
              "clb_bootresult", "clb_bootmean", "clb_bootbrd", "clb_bootrecover",
              "clb_jitterresult", "clb_jittermean", "clb_jitterbrd", "clb_jitterrecover",
              "clb_subsetresult", "clb_subsetmean", "clb_subsetbrd", "clb_subsetrecover",
              "clb_bojitresult", "clb_bojitmean", "clb_bojitbrd", "clb_bojitrecover",
              "clb_noiseresult", "clb_noisemean", "clb_noisebrd", "clb_noiserecover",
              "clb_result_nc", "clb_result_clustermethod", "clb_result_criterion", "clb_result_nccl",
              "clb_result_result_nc", "clb_result_result_crit",
              "clb_result_result_pamobject_medoids", "clb_result_result_pamobject_id.med",
              "clb_result_result_pamobject_objective", "clb_result_result_pamobject_isolation",
              "clb_result_result_pamobject_clusinfo", "clb_result_result_pamobject_diss",
              "clb_result_result_pamobject_silinfo_clus.avg.widths",
              "clb_result_result_pamobject_silinfo_avg.width",
              "features"
          ]
          cluster_singlevals_res = {}
          for x in cluster_singlevals:
              print(x)
              cluster_singlevals_res[x] = getsub(["cluster"] + x.split("_"))
      ...
      nobj[nobj.names.index("gap")][nobj[nobj.names.index("gap")].names.index("Tab")]
#+end_src
